...
def cmd_whois(message):
    ...
    try:
        ...
        if data.get("online"):
            ...
            online_players = f"{online}/{max_players}"

            # До 6 ников игроков онлайн
            player_list = players.get("list", [])
            if player_list:
                sample = player_list[:6]
                players_sample = f" ({', '.join(sample)}" + (", ..." if len(player_list) > 6 else "") + ")"
            else:
                players_sample = ""
    ...
    if server_online:
        response += (
            ...
            f"👥 <b>Онлайн игроков:</b> {online_players}{players_sample}\n"
